...
class SelfRAGOpenAI:
    ...
    def _generate_combined_response(self, query: str, contexts: List[str]) -> str:
        """
        Generate a comprehensive response using ALL relevant contexts merged.
        
        This is the key addition over standard Self-RAG — when the answer spans
        multiple chunks, individual per-context responses will each be partial.
        The combined response can synthesize information across all contexts.
        """
        numbered = "\n\n".join(
        ...
        messages = [
            {
                "role": "system",
                "content": (
                    "You are a helpful assistant. Answer the question by synthesizing "
                    "information from ALL the provided contexts. Combine relevant details "
                    "from each context into a comprehensive, well-structured answer. "
                    "Be concise but complete. If some contexts are more relevant than others, "
                    "prioritize them but include complementary details from other contexts."
                ),
            },
            {
                "role": "user",
    ...
    def _select_best(candidates: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return max(
            candidates,
            key=lambda c: (support_priority.get(c["support"], 0), c["utility"] + type_bonus.get(c["type"], 0)),
        )
    ...
    def self_rag(self, query: str) -> SelfRAGResponse:
        ...
        for i, context in enumerate(relevant_contexts):
            print(f"\n  ── Candidate {i+1}/{len(relevant_contexts)} (single context) ──")
        ...
        if len(relevant_contexts) >= 2:
            print(f"\n  ── Combined Candidate (all {len(relevant_contexts)} contexts) ──")
            
            combined_response = self._generate_combined_response(query, relevant_contexts)
            print(f"  [Generate] Combined: {combined_response[:80]}...")

            merged_context = "\n\n---\n\n".join(relevant_contexts)
            combined_support = self._assess_support(combined_response, merged_context)
            print(f"  [Support]  {combined_support}")

            combined_utility = self._evaluate_utility(query, combined_response)
            print(f"  [Utility]  {combined_utility}/5")
        ...
        candidate_type = best.get("type", "single")
        print(f"  → Winner: type={candidate_type}, support={best['support']}, utility={best['utility']}/5")

        context_used = relevant_contexts if candidate_type == "combined" else [best["context"]]
